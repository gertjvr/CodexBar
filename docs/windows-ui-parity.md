...
## Claude terminal parsing and hover submenus

CLI revision `2c18aea9b` fixes a captured Windows Claude CLI parsing failure. ConPTY
encoded spaces and row boundaries with cursor movement sequences. Removing all ANSI
sequences joined the session and weekly rows, causing both limits to read the session
percentage and losing reset descriptions. A bounded terminal screen reconstruction now
preserves cursor placement, spaces, erasure and redraws before the existing Claude parser.
Ordinary line-oriented captures retain the existing path.

The regression first failed with weekly remaining 24% instead of 91%, plus missing reset
descriptions. The fix passed 27 focused tests across terminal layout, scoped/all-models
Claude limits and status fetching. The source capture showed 76% session used and 9%
weekly used; those are separate limits, not a configuration change.

The same CLI revision adds optional `status.components` to `usage --status`, reusing Core's
status summary fetcher and provider component allowlists. Existing summary fields remain;
component-fetch failures do not erase a successful overall status result.

Tray revision `8b553355d` preserves the component tree and status timestamp in its projection.
Plan Usage, Cost and Status Page open after pointer entry with a short delay. Click and
keyboard activation remain available. Status groups show colored states and child services,
with the external status-page link below them and the dated overall status below the parent row.

Native tray build and projection tests passed on Asterix; the build took 15.54 seconds with
zero warnings/errors. WinUI CI run `34751146503` passed. `make check` reported zero violations
across 2,283 files. Desktop checks with synthetic data verified all three pointer-entry
flyouts, switching between siblings, grouped/child statuses, click activation, Escape dismissal,
Enter reopening, and keyboard selection of a different Cost day with updated model details.

The full suite passed all 1,116 selections in 93 groups in 837.6 seconds, with no failed
groups, timeouts or retries. Revision `a90a28edc` additionally keeps only the active submenu
branch; desktop testing verified replacing APIs children with ChatGPT children. Its WinUI CI
run `34751677927` passed. Revision `623aa67dc` constrains long component lists to the work area
with scrolling; the native rebuild passed in 13.03 seconds with zero warnings/errors.

Revision `a636a39a6` uses the FlyoutPresenter's single scrolling viewport. Desktop testing
reached the last of 35 synthetic API services without clipping the end of the list.
Revision `ca7c93659` removes empty icon columns from Plan Usage and Cost and replaces the
Usage Dashboard glyph with a line-chart drawing. Its native Windows build passed in 16.64
seconds with zero warnings/errors; the final `make check` reported zero violations.

Windows CLI CI run `34751134808` passed for `2c18aea9ba84`. The downloaded ZIP's SHA-256 was
verified locally and on Asterix before extraction. The corrected executable returned Claude
five-hour usage of 0% and weekly usage of 9%, with separate reset timestamps and six status
components. These are live values at validation time, not fixture results.

The matching CLI/tray pair is staged on Asterix. Installation was blocked by automatic
approval review, which classified launching the locally built executable as an unrecognized
source requiring confirmation at installation time. Final installation, installed-binary UI
checks, and visual confirmation of the last spacing/icon adjustment remain pending.


## Published preview installation

Windows Preview 2026.09.13 (`67bfb3257735`) was installed on Asterix from the published
ZIP after verifying its SHA-256. Desktop and Start menu CodexBar WinUI shortcuts point
to `%LOCALAPPDATA%/Programs/CodexBar/WinUI-67bfb3257735/CodexBar.Tray.exe`. Both installed
executables match the release artifact hashes; the companion reports `dev-67bfb3257735`.
The process remained running with no failure log. Live UI checks showed Codex usage,
Claude session 100% remaining and weekly 91% remaining with separate resets, hover-open
Claude service status details, and the hover-open Codex cost breakdown. Values are a
snapshot at validation time. The earlier installation approval block was resolved by
the owner's explicit request to install the published release.

## Provider-card hover

The Windows summary card now opens the plan-usage submenu when available, falling back
to cost history using the macOS selection order. The selected card keeps its blue
background and white text/usage fills while its submenu is open. Existing account
buttons remain separate actions. Native build passed with zero warnings/errors in
14.08 seconds; offline Windows tray tests and `make check` passed. A synthetic desktop
preview verified card hover opening the plan-usage chart and retaining selection while
moving into the chart. This change is not part of the previously published preview ZIP.
