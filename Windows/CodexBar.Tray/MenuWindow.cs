...
internal sealed class MenuWindow : Window
{
    ...
    private void Provider(JsonElement provider)
    {
        ...
        if (breakdown.ValueKind == JsonValueKind.Array && breakdown.GetArrayLength() > 0)
        {
    ...
    private static Grid Meter(double percent, string accent = "49A7B6", double? expected = null, bool remaining = true, int[]? thresholds = null)
    {
        ...
        var fill = new Border { Background = Theme.Brush(accent), CornerRadius = new(3), HorizontalAlignment = HorizontalAlignment.Left };
    ...
    private void AttachSubmenu(Button button, Action show)
    {
        ...
        button.Click += (_, _) => { timer.Stop(); show(); };
        button.KeyDown += (_, args) =>
        {
            if (args.Key is Windows.System.VirtualKey.Right or Windows.System.VirtualKey.Left)
